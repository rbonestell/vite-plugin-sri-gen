...
function mockBundle(files: Record<string, string | BundleEntry>): Bundle {
	return Object.fromEntries(
		Object.entries(files).map(([k, v]) => [k, typeof v === 'string' ? { code: v } : v])
	) as Bundle
}
...
describe('helpers', () => {
	...
	describe('normalizeBundlePath', () => {
		...
		it('strips protocol-relative prefix', () => {
			expect(normalizeBundlePath('//cdn.example.com/x.js')).toBe('cdn.example.com/x.js')
		})
	})
	...
	describe('getUrlAttrName', () => {
		it('returns src for script and href otherwise', () => {
			...
			const $l = cheerioLoad('<link rel="stylesheet" href="/a.css">')('link').get(0)
	...
	describe('loadResource', () => {
		...
		it('fetches remote when http and returns bytes', async () => {
			...
			globalThis.fetch = vi.fn().mockResolvedValue({ ok: true, arrayBuffer: () => Promise.resolve(bytes.buffer) }) as any
			const res = await loadResource('https://example.com/x.js', {}, { enableCache: false })
		...
		it('throws on failed fetch', async () => {
			globalThis.fetch = vi.fn().mockResolvedValue({ ok: false, status: 404, statusText: 'Not Found' }) as any
			await expect(loadResource('https://example.com/404.js', {})).rejects.toThrow('Failed to fetch')
		})

		it('supports protocol-relative URLs via https', async () => {
			const bytes = new Uint8Array([9, 8, 7])
			const fetchSpy = vi.fn().mockResolvedValue({ ok: true, arrayBuffer: () => Promise.resolve(bytes.buffer) })
			globalThis.fetch = fetchSpy as any
			const res = await loadResource('//cdn.example.com/lib.js', {}, { enableCache: false })
			expect(res).toBeInstanceOf(Uint8Array)
		...
		it('applies fetch timeout when configured', async () => {
			...
			globalThis.fetch = fetchSpy as any
			await expect(loadResource('https://example.com/slow.js', {}, { enableCache: false, fetchTimeoutMs: 1 }))
				.rejects.toThrow()
		})
	...
	describe('processElement', () => {
		...
		it('adds integrity and crossorigin', async () => {
			const $script = $('script')
			await processElement($script, mockBundle({ 'a.js': 'console.log(1)' }), 'sha256', 'anonymous')
	...
	describe('addSriToHtml', () => {
		...
		it('adds integrity to multiple element types', async () => {
			...
			const out = await addSriToHtml(html, mockBundle({ 'style.css': 'body{}', 'entry.js': 'x' }), { algorithm: 'sha384' })
		...
		it('skips missing bundle resources without warning', async () => {
			...
			const out = await addSriToHtml(html, mockBundle({}), { algorithm: 'sha256' })
		...
		it('respects crossorigin option', async () => {
			...
			const out = await addSriToHtml(html, mockBundle({ 'a.js': 'x' }), { algorithm: 'sha256', crossorigin: 'anonymous' } as any)
		...
		it('resolves bundle items via endsWith match', async () => {
			...
			const bundle = mockBundle({ 'dist/assets/nested/path/app.js': 'console.log(1)' })
		...
		it('resolves bundle items via basename fallback', async () => {
			...
			it('warns per element when processing fails', async () => {
				...
				try {
					globalThis.fetch = vi.fn().mockResolvedValue({ ok: false, status: 500, statusText: 'ERR' }) as any
