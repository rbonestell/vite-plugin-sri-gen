...
describe('DOM Abstraction Layer', () => {
	...
	describe('DOMAdapter', () => {
		it('identifies element types correctly with call tracking', () => {
			...
			expect(dependencies.domAdapter.isHTMLScriptElement(script)).toBe(true);
			expect(dependencies.domAdapter.isHTMLLinkElement(link)).toBe(true);
			expect(dependencies.domAdapter.isHTMLScriptElement(div)).toBe(false);
			expect(dependencies.domAdapter.isHTMLLinkElement(div)).toBe(false);
			
			// Verify call tracking
			expect(dependencies.mocks.domAdapter.getScriptElementCalls()).toContain(script);
			expect(dependencies.mocks.domAdapter.getScriptElementCalls()).toContain(div);
			expect(dependencies.mocks.domAdapter.getLinkElementCalls()).toContain(link);
			expect(dependencies.mocks.domAdapter.getLinkElementCalls()).toContain(div);
		});

		it('checks SRI eligibility correctly with Vitest spy verification', () => {
			const scriptWithSrc = mockElements.createScript({ src: '/test.js' });
			const scriptWithoutSrc = mockElements.createScript();
			const scriptWithIntegrity = mockElements.createScript({ src: '/test.js', integrity: 'sha256-abc' });
			
			const stylesheetLink = mockElements.createLink({ href: '/test.css', rel: 'stylesheet' });
			const modulepreloadLink = mockElements.createLink({ href: '/test.js', rel: 'modulepreload' });
			const preloadScript = mockElements.createLink({ href: '/test.js', rel: 'preload', as: 'script' });
			const nonEligibleLink = mockElements.createLink({ href: '/test.html', rel: 'next' });

			expect(dependencies.domAdapter.isEligibleForSRI(scriptWithSrc)).toBe(true);
			expect(dependencies.domAdapter.isEligibleForSRI(scriptWithoutSrc)).toBe(false);
			expect(dependencies.domAdapter.isEligibleForSRI(scriptWithIntegrity)).toBe(false);
			expect(dependencies.domAdapter.isEligibleForSRI(stylesheetLink)).toBe(true);
			expect(dependencies.domAdapter.isEligibleForSRI(modulepreloadLink)).toBe(true);
			expect(dependencies.domAdapter.isEligibleForSRI(preloadScript)).toBe(true);
			expect(dependencies.domAdapter.isEligibleForSRI(nonEligibleLink)).toBe(false);
			
			// Verify spy was called 7 times
			expect(dependencies.mocks.domAdapter.isEligibleForSRI).toHaveBeenCalledTimes(7);
		});
		...
		it('extracts URLs correctly with spy verification', () => {
			...
			expect(dependencies.domAdapter.getElementURL(script)).toBe('/test.js');
			expect(dependencies.domAdapter.getElementURL(link)).toBe('/test.css');
			expect(dependencies.domAdapter.getElementURL(div)).toBe(null);
			
			// Verify spy calls
			expect(dependencies.mocks.domAdapter.getElementURL).toHaveBeenCalledTimes(3);
			expect(dependencies.mocks.domAdapter.getElementURL).toHaveBeenCalledWith(script);
			expect(dependencies.mocks.domAdapter.getElementURL).toHaveBeenCalledWith(link);
			expect(dependencies.mocks.domAdapter.getElementURL).toHaveBeenCalledWith(div);
		});

		it('sets integrity attributes with Vitest spy verification', () => {
			const element = mockElements.createScript({ src: '/test.js' });
			
			dependencies.domAdapter.setIntegrityAttributes(element, 'sha256-abc123', 'anonymous');
			...
			expect(dependencies.mocks.domAdapter.setIntegrityAttributes).toHaveBeenCalledTimes(1);
			expect(dependencies.mocks.domAdapter.setIntegrityAttributes).toHaveBeenCalledWith(
				element, 'sha256-abc123', 'anonymous'
			);
		});
		...
		it('handles setAttribute failures gracefully', () => {
			...
			expect(() => {
				dependencies.domAdapter.setIntegrityAttributes(element, 'sha256-abc123');
			}).not.toThrow();
	...
	describe('URLAdapter', () => {
		it('resolves URLs correctly with Vitest spy verification', () => {
			...
			// Verify spy calls
			expect(dependencies.mocks.urlAdapter.resolveURL).toHaveBeenCalledTimes(2);
			expect(dependencies.mocks.urlAdapter.resolveURL).toHaveBeenCalledWith('/test.js');
			expect(dependencies.mocks.urlAdapter.resolveURL).toHaveBeenCalledWith('test.js');
		});
		...
		it('handles URL parsing failures with spy tracking', () => {
			...
			expect(dependencies.mocks.urlAdapter.resolveURL).toHaveBeenCalledWith('invalid://url');
		});
		...
		it('tracks getBaseURL calls with Vitest spy', () => {
			...
			// Verify spy was called
			expect(dependencies.mocks.urlAdapter.getBaseURL).toHaveBeenCalledTimes(1);
		});
	});
	...
	describe('NodeAdapter', () => {
		it('tracks wrapping calls with Vitest spy', () => {
			...
			dependencies.nodeAdapter.wrapNodeInsertion(mockProto, 'testMethod', callback);
			
			// Verify spy was called with correct arguments
			expect(dependencies.mocks.nodeAdapter.wrapNodeInsertion).toHaveBeenCalledTimes(1);
			expect(dependencies.mocks.nodeAdapter.wrapNodeInsertion).toHaveBeenCalledWith(
				mockProto, 'testMethod', callback
			);
		});
		...
		it('wraps setAttribute correctly with spy verification', () => {
			...
			// Verify spy was called
			expect(dependencies.mocks.nodeAdapter.wrapSetAttribute).toHaveBeenCalledTimes(1);
			expect(dependencies.mocks.nodeAdapter.wrapSetAttribute).toHaveBeenCalledWith(
				mockProto, callback
			);
			
			const testElement = mockElements.createScript();
			// Ensure the element has tagName property for URL attribute detection
			Object.defineProperty(testElement, 'tagName', { value: 'script', configurable: true });
		...
		it('restores prototypes after testing', () => {
			...
			dependencies.nodeAdapter.wrapNodeInsertion(mockProto, 'testMethod', () => {});
...
describe('Runtime Installation with Dependency Injection', () => {
	...
	it('installs runtime with injected dependencies', () => {
		...
		const mockNode = { prototype: { appendChild: () => {}, insertBefore: () => {} } };
		...
		try {
			...
			// Should not throw
			expect(() => {
				installSriRuntimeWithDeps(sriMap, { crossorigin: 'anonymous' }, dependencies);
			}).not.toThrow();
			
			// Verify that adapters were used through Vitest spies
			expect(dependencies.mocks.nodeAdapter.wrapSetAttribute).toHaveBeenCalledTimes(1);
			expect(dependencies.mocks.nodeAdapter.wrapNodeInsertion).toHaveBeenCalledTimes(4); // Updated to match actual calls
		} finally {
	...
	it('processes elements correctly through the runtime', () => {
		...
		if (dependencies.domAdapter.isEligibleForSRI(script)) {
			...
			if (integrity) {
				dependencies.domAdapter.setIntegrityAttributes(script, integrity, 'anonymous');
			}
		}
...
describe('Production DOMAdapter', () => {
	...
	describe('isEligibleForSRI', () => {
		...
		it('handles script elements correctly', () => {
			const createMockElement = (attrs: Record<string, string>, tagName = 'script') => ({
			...
			expect(adapter.isEligibleForSRI(createMockElement({ src: '/test.js' }))).toBe(true);
			...
			expect(adapter.isEligibleForSRI(createMockElement({ src: '/test.js', integrity: 'sha256-abc' }))).toBe(false);
		});

		it('handles link elements correctly', () => {
			const createMockElement = (attrs: Record<string, string>, tagName = 'link') => ({
				hasAttribute: (name: string) => name in attrs,
				getAttribute: (name: string) => attrs[name] || null,
				tagName,
				nodeName: tagName.toUpperCase()
			...
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.css', rel: 'stylesheet' }))).toBe(true);
			
			// Modulepreload link - eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.js', rel: 'modulepreload' }))).toBe(true);
			
			// Preload script - eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.js', rel: 'preload', as: 'script' }))).toBe(true);
			
			// Preload style - eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.css', rel: 'preload', as: 'style' }))).toBe(true);
			
			// Preload font - eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.woff2', rel: 'preload', as: 'font' }))).toBe(true);
			
			// Preload other - not eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.txt', rel: 'preload', as: 'document' }))).toBe(false);
			
			// Link without href - not eligible
			expect(adapter.isEligibleForSRI(createMockElement({ rel: 'stylesheet' }))).toBe(false);
			
			// Link with integrity - not eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.css', rel: 'stylesheet', integrity: 'sha256-abc' }))).toBe(false);
			
			// Non-eligible rel type - not eligible
			expect(adapter.isEligibleForSRI(createMockElement({ href: '/test.html', rel: 'next' }))).toBe(false);
		});
		...
		it('handles other element types', () => {
			...
			expect(adapter.isEligibleForSRI(createMockElement('div'))).toBe(false);
			expect(adapter.isEligibleForSRI(createMockElement('img'))).toBe(false);
		});

		it('handles elements with missing tagName gracefully', () => {
			const element = {
				hasAttribute: (name: string) => name === 'src',
				getAttribute: (name: string) => name === 'src' ? '/test.js' : null,
				nodeName: 'SCRIPT'
			} as IBrowserElement;
	...
	describe('setIntegrityAttributes', () => {
		it('sets integrity and crossorigin attributes successfully', () => {
			...
			adapter.setIntegrityAttributes(element, 'sha256-abc123', 'anonymous');
	...
	describe('getElementURL', () => {
		...
		it('returns href for link elements', () => {
			const element = {
				tagName: 'link',
				getAttribute: (name: string) => name === 'href' ? '/test.css' : null
			} as IBrowserElement;
...
describe('Production NodeAdapter', () => {
	...
	describe('wrapNodeInsertion', () => {
		...
		it('wraps method and processes inserted elements', () => {
			const originalMethod = vi.fn((child: any) => `appended ${child.tagName}`);
	...
	describe('wrapSetAttribute', () => {
		...
		it('wraps setAttribute and processes URL-changing attributes', () => {
			...
			expect(originalSetAttribute).toHaveBeenCalledWith('src', '/test.js');
...
describe('Production URLAdapter', () => {
	...
	describe('resolveURL', () => {
		it('resolves URLs using provided base URL', () => {
			const result = adapter.resolveURL('/test.js', 'http://example.com/app/');
		...
		it('handles URL constructor throwing errors', () => {
			...
			try {
				const result = adapter.resolveURL('test.js', 'http://example.com/');
				expect(result).toBe('test.js');
			} finally {
...
describe('Default Dependencies', () => {
	...
	it('has working DOM adapter', () => {
		...
		expect(defaultDependencies.domAdapter.isEligibleForSRI(mockElement)).toBe(false);
	});

	it('has working URL adapter', () => {
		const result = defaultDependencies.urlAdapter.resolveURL('/test.js', 'http://example.com/');
		expect(result).toBe('http://example.com/test.js');
	});
});