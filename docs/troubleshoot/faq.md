...
## What does "No emitted HTML detected during SSR build" mean?
...
## My CSP blocks the injected import map
...
## I passed an invalid algorithm — what happens?
...
Vite and modern Node tooling are native ESM-first. Dropping CommonJS simplifies the package and aligns with Vite's own expectations. Import the plugin with a standard ESM import:
...
If you need to exclude a specific element so its existing attributes survive the build unchanged, add it to `skipResources`. Elements that match a skip pattern are not processed at all — they keep every attribute exactly as written.

```ts
sri({
  skipResources: ['#my-element', 'https://cdn.example.com/pinned-file.js'],
})
```
