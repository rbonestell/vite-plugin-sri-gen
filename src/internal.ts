...
function findBundleItem(bundle: BundleLike, relPath: string): BundleItem | null {
  if (!bundle) return null;
  const keys = Object.keys(bundle);
  // Exact match
  if ((bundle as Record<string, BundleItem>)[relPath]) return (bundle as Record<string, BundleItem>)[relPath];
...
export function computeIntegrity(source: string | Uint8Array, algorithm: "sha256" | "sha384" | "sha512"): string {
...
export async function addSriToHtml(
  ...
  await Promise.all(
    $elements
      ...
      .map((node) => $(node))
      .map(($node) =>
        processElement($node, bundle, algorithm, crossorigin, resourceOpts).catch((err: any) => {
